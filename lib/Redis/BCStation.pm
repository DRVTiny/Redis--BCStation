...
use 5.16.1;
use subs qw/__get_timer_settings/;
use EV;
use Mojo::Redis2;
use Mojo::IOLoop;
use Log::Log4perl;
    ...
    %props=(
        ...
        'keep_alive'=>{
            'val' => sub {
                ...
                $keepAliveTimings=
                    (!int(ref $_[0]) and $_[0] eq '1')
                        ? $dfltTimings
                        : __get_timer_settings($_[0], $dfltTimings)
                            || $log->logdie('Keep-alive settings must be "[after:]interval" string or ref to the list containing maximum 2 numeric elements');
