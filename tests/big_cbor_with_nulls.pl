...
my $data_pub=encodeByTag('CB', 'Z', 'B64' => JSON::XS->new->decode(read_file(shift // die 'You must specify file path')));
my ($l_data_pub, $md5_data_pub) = (length($data_pub), md5_hex($data_pub));
#say("$l_data_pub, $md5_data_pub"), exit;
log_level('TRACE');
info_ 'Lets start our game';
my $bcst=Redis::BCStation->new('galaxyNews', 'keep_alive' => 1, 'fast_but_binary_unsafe' => 1);

$bcst->subscribe('warNeverChanges', 
sub {
    my ($l_data_rcvd, $md5_data_rcvd) = ( length($_[0]), md5_hex($_[0]) );
    info_ "Received data with length=%s and md5sum=%s\n", $l_data_rcvd, $md5_data_rcvd;
